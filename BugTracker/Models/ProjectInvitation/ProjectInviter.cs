...
public class ProjectInviter : IProjectInviter
{
	...
	public async Task AddProjectInvitation(ProjectInvitation invitation)
	{
		...
		var userAlreadyExists = invitation.ToUser != null;
		var emailAddressIsPendingRegistration = await EmailAddressPendingRegistration(invitation.EmailAddress, invitation.Project.ProjectId);
		...
		{
			if(!await UserHasProjectAuthorization(invitation.EmailAddress, invitation.Project.ProjectId))
			{
	...
	{
		var emailSubject = $"Invitation to join project: { invitation.Project.Name }";
		var emailMessage = $"You have been invited by { invitation.FromUser.UserName } to join the project { invitation.Project.Name } as a member.";
	...
	{
		var emailSubject = $"Membership added to project: { invitation.Project.Name }";
		var emailMessage = $"Dear { invitation.ToUser.UserName},\n" +
